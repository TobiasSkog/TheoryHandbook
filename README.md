...
## .NET
**.NET** är ett verktyg för utvecklare, ett så kallat [[framework]], som Microsoft utvecklade. 
**.NET** har ett många verktyg och en uppsjö av [[Teorihandboken - Svar#^18fbab|libraries]] som underlättar utvecklandet av applikationer riktade mot webben, mobiler och datorer.

**.NET Core** är ett så kallat [[#^420e6e|cross-platform]] [[Teorihandboken - Svar#^30c3b1|framework]] som tillåter utvecklaren att lätt skapa produkter för olika typer av operativsystem (Windows, Linux, macOS, iOS, Android).

**.NET Core** 1.0 släpptes i juni 2016 och sedan dess har .NET vart open-source!

**.NET** använder sig av **CLR (Common Language Runtime)** vilket är en implementation av **[[#^fac073||CLI (Common Language Infrastructure)]]** som är en internationell standard. Detta innebär att att **.NET** kan köra kod och libraries som är skrivna i olika programmeringsspråk men är kompatibel med **.NET's** [[#^dfb82d|CLR]] och [[#^fac073|CLI]]. **.NET's** [[#^dfb82d|CLR]] använder sig av **[[#^d5a167|JIT (Just In Time)]]** kompilering som konverterar [[#^eedc37|IL]] koden till instruktioner som datorn förstår ([[#^480bf0||Binär kod]]). Språkinteroperabilitet är en nyckelfunktionen i **NET** [[#^eedc37|IL]] koden producerad av C# kompilatorn överensstämmer med **[[#^2206af|CTS (Common Type Specification)]]**.
Vilket innebär att IL kod som är genererad i C# kan interagera med annan kod som har är kompatibel med samma CTS som **.NET** kan tyda, dvs Du är inte begränsad till att enbart använda dig av programmeringsspråket C# då du som utvecklare använder dig av **.NET** och så länge språket är kompatibelt med **.NET** [[#^2206af|CTS]] så kan kod i olika programmeringsspråk kommunicera med varandra utan avbrott eller kostnad av hastighet eller prestanda.
